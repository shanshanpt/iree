...
class AddVulkanLaunchWrapper
 ...
 private:
  ...
  /// Add an entry point with the matching function signature
...
void AddVulkanLaunchWrapper::convertGpuLaunchFunc(
  ...
  auto arg = MemRefType::get({4, 4}, FloatType::getF32(ctx), {}, 0);
  FunctionType ft = FunctionType::get({arg, arg, arg}, {}, ctx);
  ...
  args.insert(args.end(), function.args_begin(), function.args_end());

  // Create vulkan launch call op.
  auto vulkanLaunchCallOp = builder.create<CallOp>(
      loc, ArrayRef<Type>{}, builder.getSymbolRefAttr(kVulkanLaunch), args);
...
/// matching signature.
class SetSpirvABI
    : public PassWrapper<SetSpirvABI, OperationPass<spirv::ModuleOp>> {
 public:
  void runOnOperation() override {
    spirv::ModuleOp module = getOperation();
    MLIRContext *context = &getContext();
    for (auto f : module.getOps<spirv::FuncOp>()) {
...
mlir::createAddVulkanLaunchWrapper(int64_t workload, ArrayRef<Type> args) {
  return std::make_unique<AddVulkanLaunchWrapper>(workload, args);
}

std::unique_ptr<mlir::OperationPass<mlir::spirv::ModuleOp>>
mlir::createSetSpirvABI() {
